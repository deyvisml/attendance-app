...
import { ActivityIndicator, Alert, FlatList, SafeAreaView, StyleSheet, Text, View, RefreshControl, Pressable, Platform, Modal, ScrollView  } from 'react-native';
...
function alertJson(title: string, payload: any, max = 1200) {
  ...
  if (typeof payload === 'string') {
    ...
    try { msg = JSON.stringify(payload, null, 2); }
    catch { msg = String(payload); }
  }
...
//   const [marking, setMarking] = useState(false);   // spinner al hacer POST
    ...
    async function load() {
        ...
        try {
        setLoading(true);
        const res = await fetch(`${ENDPOINT}/api/attendance-lists/${id}/registered-students`, {
        ...
        const mapped: Row[] = (json?.data ?? []).map((it, idx) => {
            ...
            const fullName = [u.name, u.father_last_name, u.mother_last_name].filter(Boolean).join(' ');
            return {
        ...
        Alert.alert('Error', e?.message ?? 'No se pudo cargar los estudiantes registrados.');
        } finally {
        setLoading(false);
        setRefreshing(false);
        }
  ...
  async function markAttendance(codeRaw: string) {
    if (!ENDPOINT) { Alert.alert('Config', 'Falta EXPO_PUBLIC_API_URL'); return; }

    const listIdNum = Number(id);
    if (!Number.isFinite(listIdNum)) { Alert.alert('Error', 'ID de lista inválido.'); return; }
    ...
    try {
      ...
      if (!res.ok) {
        if (res.status === 422 && payload?.errors) {
          ...
          const msg = payload.errors[firstKey]?.[0] ?? payload.message ?? 'Error de validación.';
          throw new Error(msg);
        }
  ...
  function handleScan({ data }: { data: string }) {
    ...
    if (data === lastCodeRef.current && (now - lastSameCodeAtRef.current) < DEDUPE_WINDOW_MS) return;
  ...
  const rightBtnLabel = busy ? 'Procesando…' : (enabled ? 'Pausar' : 'Reanudar');
      return (
        ...
        {/* <View style={{ padding: 16 }}> */}
          <Text style={{ paddingTop: 16, fontSize: 20, fontWeight: '700', textAlign: "center" }}>{title}</Text>
          <FlatList
              data={items}
              ...
              ListEmptyComponent={
              <View style={s.empty}>
                  <Text>No hay estudiantes registrados todavía.</Text>
              </View>
              }
              renderItem={({ item }) => (
                ...
                <View style={s.row}>
                    ...
                    <Text style={s.name}>sdfsdfsdfsd</Text>
                    <Text style={s.meta}>
                        DNI: {item.dni || '—'} · {item.grade} — Sección {item.section || '—'}
                    </Text>
                    <Text style={s.time}>
                        {new Date(item.datetime.replace(' ', 'T')).toLocaleString()}
                    </Text>
                    </View>
                </View>
            ...
            <View style={s.scanBackdrop}>
              <View style={s.scanCard}>
                  ...
                  <View style={s.noPerms}>
                      ...
                      <Pressable onPress={requestPermission} style={[s.scanBtn, s.primary]}>
                  ...
                  <View style={s.scanRow}>
                    ...
                    <Pressable
                      ...
                      disabled={busy}
                    >
                      <Text style={[s.btnText, isResumeState ? s.dangerText : null]}>{rightBtnLabel}</Text>
                    </Pressable>
                  </View>
              </View>
            </View>
...
const s = StyleSheet.create({
  ...
  empty: { padding: 0, alignItems: 'center' },
  row: {
    // paddingVertical: 10,
    paddingHorizontal: 8,
  ...
  time: { color: '#6b7280', marginTop: 2, fontSize: 12 },
  sep: { height: 8 },
  fabBase: {
    position: 'absolute',
    width: 56,
